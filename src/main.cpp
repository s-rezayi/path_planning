      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for(int i = 0; i < sensor_fusion.size(); i++)
          {
            ...
            // Estimate S-coordinate of other vehile
          ...
          if(too_close) 
          {
            if(!left_occupied && lane > 0)
            {
              lane--;
            }
            else if(!right_occupied && lane != 2)
            {
              lane++;
            }
            else
          ...
          // Next fernet coordinates
          double dist_ahead = 30.0;
          double next_s0 = car_s + dist_ahead;
          double next_s1 = next_s0 + dist_ahead;
          double next_s2 = next_s1 + dist_ahead;
          double next_d = 2 + 4 * lane;

          vector<double> next_wp0 = getXY(next_s0, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          vector<double> next_wp1 = getXY(next_s1, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          vector<double> next_wp2 = getXY(next_s2, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
